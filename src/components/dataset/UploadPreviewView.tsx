...
export function UploadPreviewView({ onConfirm }: Props) {
  const { datasetRows, setDatasetRows, referenceColumn, setReferenceColumn } = useEval();
  const columns = datasetRows.length > 0 ? Object.keys(datasetRows[0]).filter(k => k !== '_id') : [];
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative w-64">
              <select 
                ...
                className="w-full bg-[#111] border border-[#333] rounded-md pl-4 pr-10 py-2 text-sm text-zinc-200 appearance-none focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
              >
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="border-b border-[#222] bg-[#111]">
                  ...
                  {columns.map(col => (
                    <th key={col} className="py-3 px-4 text-xs font-medium text-zinc-500 uppercase tracking-wider">
                      {col === referenceColumn ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20 mr-2">{col}</span>
                      ) : col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-[#222] text-sm text-zinc-300">
                {datasetRows.length === 0 ? (
                  <tr className="hover:bg-[#111]/50 transition-colors">
                    <td colSpan={columns.length + 1} className="py-8 px-4 text-center text-zinc-500 text-sm">
                  ...
                  datasetRows.map((row, i) => (
                    <tr key={row._id} className="hover:bg-[#111]/50 transition-colors">
                      ...
                      {columns.map(col => (
                        <td key={col} className={`py-3 px-4 text-xs ${col === referenceColumn ? '' : 'text-zinc-400'}`}>
                          {String(row[col])}
                        </td>
                      ))}
                    </tr>
